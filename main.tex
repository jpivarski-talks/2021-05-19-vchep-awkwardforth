...
\uncover<2->{\underline{Potential solutions}}
...
\vspace{\baselineskip}
\vspace{0.25 cm}
\uncover<4->{Create a lightweight/specialized virtual machine.}

\column{0.45\linewidth}
\uncover<2->{\underline{Suitable for Uproot?}}
...
\vspace{0.25 cm}
\uncover<4->{That's the subject of this talk.}
\end{columns}
\end{frame}
...
\vspace{0.4 cm}
\uncover<3->{Python's \mintinline{python}{struct} module (byte-parsing) and \mintinline{python}{numexpr} (math) are similar.}
\end{frame}

\begin{frame}{Another issue: keeping Uproot and Awkward Array independent}
